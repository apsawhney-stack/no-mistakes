...
func PublishFinalWorkAttestationToPR(ctx context.Context, sctx *pipeline.StepContext, att *types.WorkAttestation) error {
	if sctx == nil || sctx.Run == nil || att == nil || sctx.DB == nil {
		return nil
	}
	prURL := strings.TrimSpace(runPRURL(sctx))
	if prURL == "" {
		return nil
	}
	provider := resolvedProvider(sctx)
	...
	if host == nil {
		return fmt.Errorf("build PR attestation host: %s", reason)
	}
	if err := host.Available(ctx); err != nil {
		return fmt.Errorf("PR attestation host unavailable: %w", err)
	}
	...
	return restampPRAttestationWithSteps(ctx, host, &scm.PR{URL: prURL}, att.FinalHeadSHA, steps, sctx.Log, policy)
}
