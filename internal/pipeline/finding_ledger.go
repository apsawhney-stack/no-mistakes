	...
	for _, e := range unresolved {
		...
		if ciSettledClean && e.Status != types.FindingLedgerStatusPendingVerification && supersededByCICleanObservation(e) {
			...
			if err := l.db.UpdateFindingLedgerEntryWithEvent(e, ev); err != nil {
				return outcome.Findings, fmt.Errorf("supersede finding ledger entry %s for step %s: %w", e.ID, stepName, err)
			}
			continue
		}
...
// (pending_verification), one imported from pre-ledger history whose outcome
// cannot be proven (needs_reconciliation), or an open ledger entry that terminal
// acceptance would refuse as blocking.
//
// Terminal acceptance refuses pending and reconciliation entries
// unconditionally, and refuses open entries when the ledger summary says an
// unresolved entry is blocking by types.IsBlockingFinding. Those entries must
// reach a gate rather than dead-ending the run. An open entry that is explicitly
// non-blocking - an informational no-op note - keeps the same semantics it had
// before the ledger existed and must not hold its step at a gate.
func ledgerRequiresDisposition(raw string) bool {
	...
	return parsed.Ledger.PendingCount > 0 || parsed.Ledger.ReconcileCount > 0 || (parsed.Ledger.OpenCount > 0 && parsed.Ledger.HasBlocking)
}
...
func (l *FindingLedger) ProcessSelection(
	...
	for _, e := range entries {
		...
		if matched[e.ID] && e.Status == types.FindingLedgerStatusOpen {
			...
			if err := l.db.UpdateFindingLedgerEntryWithEvent(e, ev); err != nil {
				return fmt.Errorf("select finding ledger entry %s for step %s: %w", e.ID, stepName, err)
			}
		}
	}
	return nil
...
func (l *FindingLedger) RecordCorrectingRevision(
	...
	for _, e := range entries {
		if e.Status == types.FindingLedgerStatusPendingVerification {
			...
			if err := l.db.UpdateFindingLedgerEntryWithEvent(e, ev); err != nil {
				return fmt.Errorf("record correcting revision finding ledger entry %s in step %s: %w", e.ID, stepName, err)
			}
		}
	}
	return nil
...
func (l *FindingLedger) ProcessExplicitDispositionForEntries(
	...
	for _, e := range entries {
		...
		if err := l.db.UpdateFindingLedgerEntryWithEvent(e, ev); err != nil {
			return fmt.Errorf("dispose finding ledger entry %s for step %s: %w", e.ID, stepName, err)
		}
	}
	return nil
}
