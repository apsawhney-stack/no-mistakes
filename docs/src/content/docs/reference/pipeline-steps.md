...
Findings the durable protocol owns are tracked in a versioned per-run ledger until something positively disposes of them. [Finding Ledger Protocol](/no-mistakes/reference/finding-ledger/) owns entry identity, statuses, closure proofs, CI check supersession, step-owned exceptions, and legacy import.

When a human resolves a findings gate with Approve, Skip, or Abort without selecting a fix, no-mistakes records that the round's findings were declined and closes unresolved ledger entries with that explicit disposition. A gate with no findings records no decision. When the human selects findings to fix, the selection is recorded for the fixer and the ledger; unselected findings stay unresolved until a later verified closure or explicit disposition. Findings merely left out by automatic filtering remain undecided.
...
- Carries unresolved Review findings through the [finding ledger](/no-mistakes/reference/finding-ledger/) across fix rounds. Selecting a finding for a fix hands it to the fixer but does not remove it; Review findings clear only through the ledger's verified-closure proof or an explicit operator disposition. A selected `info` finding can therefore keep the gate parked even though an unselected informational `no-op` finding is non-blocking
...
**Approval:** required if any finding has severity `error` or `warning`, any finding has `action: ask-user`, or a Review fix round leaves a selected finding outstanding. The last condition applies regardless of severity; an unselected informational `no-op` finding remains non-blocking. Findings with `action: ask-user` pause for approval instead of entering the normal auto-fix loop. This is for findings that challenge the author's intent, or whose smallest honest remedy would extend the change rather than correct it, not routine correctness, reliability, or security fixes that may need to re-add a small amount of deleted logic. With the default `auto_fix.review: 0`, blocking review findings park for approval even when their action is `auto-fix`; setting repo or global `auto_fix.review > 0` re-enables the automatic review fix loop for eligible `auto-fix` findings. Findings with `action: no-op` are informational only. The shared [finding-action model](/no-mistakes/concepts/auto-fix/#finding-actions) owns the behavior for a missing `action`.
